...
export default function ServicesOurGoal() {
  const { t, img, config, cp } = useHayc();
  const g = config.servicesPageConfig.ourGoal;
          ...
          <div className="text-wrapper mb-0 w-full lg:w-1/2">
            <div className="section-title mb-6">
              ...
              <h2
                ...
                {...cp('servicesPageConfig.ourGoal.title')}
              >
                {t(g.title)}
              </h2>
              <p
                className="blue-dark mt-3 font-medium text-blue-900"
                {...cp('servicesPageConfig.ourGoal.lead')}
              >
                {t(g.lead)}
              </p>
            </div>
            {g.paragraphs.map((para, i) => (
              <p
                ...
                {...cp(`servicesPageConfig.ourGoal.paragraphs.${i}`)}
              >
                {t(para)}
              </p>
            ))}
            <AppLink
              ...
              {...cp('servicesPageConfig.ourGoal.ctaLabel')}
            >
              {t(g.ctaLabel)}
            </AppLink>
          </div>
          <div className="img-holder relative z-[1] w-full lg:w-1/2">
            <div className="shape shape-two pointer-events-none absolute -right-4 -top-6 z-[2] w-24 opacity-90 md:w-32">
              <img
                ...
                {...cp('servicesPageConfig.ourGoal.shapeImage')}
              />
            </div>
            <div className="relative overflow-hidden rounded-xl shadow-lg ring-1 ring-black/5">
              <img
                ...
                {...cp('servicesPageConfig.ourGoal.mainImage')}
              />
            </div>
          </div>
        </div>
